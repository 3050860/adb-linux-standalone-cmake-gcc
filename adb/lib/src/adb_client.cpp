...
#include "../lib/include/AdbDevice.h"

// Forward declaration
class AdbDevice;

// Thread-local указатель на наше устройство. По умолчанию nullptr.
thread_local AdbDevice* g_current_adb_device = nullptr;

// Функция для установки устройства из нашего кода
extern "C" void adb_set_current_device(AdbDevice* device) {
    g_current_adb_device = device;
}

thread_local static TransportType __adb_transport = kTransportAny;
thread_local static const char* __adb_serial = nullptr;
thread_local static TransportId __adb_transport_id = 0;
...
int adb_connect(TransportId* transport, std::string_view service, std::string* error, bool force_switch_device) {
    ...
    // ПЕРЕХВАТ: Если установлено наше устройство, создаем сессию через него
    if (g_current_adb_device) {
        int fd = g_current_adb_device->connectServiceSync(std::string(service));
        if (fd < 0) {
            if (error) *error = "Failed to connect service via AdbDevice";
            return -1;
        }
        return fd;
    }
...
//     thread_local static std::optional<FeatureSet> features [[clang::no_destroy]] GUARDED_BY(feature_mutex);
