...
public:
    ...
    int releaseFd() { return local_fd_.release(); }
    ...
    // Сессию при этом НЕ прерывает: метод рассчитан и на ожидание порциями,
    // решение «рвать через abort() или ждать дальше» остаётся за вызывающим
    // (§14 п.4). timeout_ms == 0 — ждать бесконечно, как wait().
