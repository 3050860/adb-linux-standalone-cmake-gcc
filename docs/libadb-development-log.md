...
## Этап 10 — авторизация: ключи из файлов и из текста PEM (§5)

Первый этап после пересмотра плана (см. врезку о нумерации в начале файла).

**Что сделано**

Публичный API:

- `AuthOptions{key_files, private_keys_pem, use_default_key_store,
  generate_ephemeral_if_empty, save_generated_key_to}` и `Options::auth`.
- `libadb::auth_key_fingerprints()` — отпечатки (SHA-256, hex) загруженных
  приватных ключей: диагностика «каким ключом мы вообще стучимся».
- Ошибка разбора ключа → `Status::InvalidArgument` из `Client::initialize()`,
  в `LogSink` уходит сообщение с указанием, какой именно ключ не разобрался.

Внутренний слой (`adb/client/auth.cpp`, `adb/adb_auth.h`):

- `load_key_from_pem()` — загрузка приватного ключа из строки через
  `BIO_new_mem_buf`. Поддержаны оба формата: `BEGIN RSA PRIVATE KEY` и PKCS#8
  `BEGIN PRIVATE KEY` (второй — через `PEM_read_bio_PrivateKey` +
  `EVP_PKEY_get1_RSA`), потому что вызывающий может дать любой.
- `adb_auth_init_ex(AdbAuthConfig)` — настраиваемая инициализация: стандартный
  набор ключей включается/выключается, добавляются файлы и PEM-строки, при
  пустом наборе генерируется эфемерный ключ (с опциональной записью на диск
  вместе с `.pub`).
- `adb_auth_is_configured()`, `adb_auth_key_count()`,
  `adb_auth_key_fingerprints()`.
- `AdbManager::start()` больше не зовёт `adb_auth_init()` безусловно.

**Принятые решения и грабли**

1. **Флаг «настроено», а не «ключей ноль».** Сначала `AdbManager::start()`
   проверял `adb_auth_key_count() == 0` — и в конфигурации
   «`use_default_key_store=false`, своих ключей нет, генерация запрещена» (вполне
   законной: пусть устройство ответит `AuthRequired`) подмешивал стандартный
   набор поверх явно заданного. Тест это поймал: в наборе оказывался чужой ключ.
   Теперь есть отдельный `adb_auth_is_configured()`.
2. **`adb_auth_inotify_init()` — ровно один раз за процесс.** `Client::initialize()`
   допускает повторные вызовы (например, чтобы поменять таймауты), а слежение за
   каталогами ключей делает `fdevent_create()`. Второй вызов при уже работающем
   event loop — это `fdevent_create` из чужого потока и аварийное завершение
   процесса; ровно так и упал `test_012`, который переинициализирует клиент
   четыре раза. Обёрнуто в `std::call_once`.
3. **Авторизация настраивается строго до старта event loop.** Рукопожатие
   начинается сразу после `connect()`, ключи к этому моменту должны быть на месте.
4. **Дедупликация по отпечатку досталась бесплатно:** ключи лежат в
   `map<fingerprint, RSA>`, поэтому один и тот же ключ, поданный и файлом, и
   текстом, набор не удваивает (проверено тестом).
5. **`CalculatePublicKey()` принимает `RSA*`, а не `EVP_PKEY*`** — на этом
   сначала не собралось; для записи сгенерированного ключа используется уже
   извлечённый `RSA*`.

**Что проверено (`test/auto/test_018_auth.cpp`, 20 проверок, ALL PASSED на
192.168.177.249)**

Набор ключей внутри adb глобален и не выгружается, поэтому подтесты, которым
нужен чистый набор, тест запускает **отдельными процессами** (`--mode=...`) и
пробрасывает их вывод в общий отчёт:

- `ephemeral`: `use_default_key_store=false` + генерация → ровно один ключ,
  приватный и `.pub` записаны, в файле действительно PEM;
- `badpem`: испорченный PEM → `Status::InvalidArgument`, битый ключ в набор не
  попал;
- `nokeys`: ключей нет и генерация запрещена → `initialize()` проходит
  (`Status::Ok`), набор **пуст** — тот самый случай из грабли 1;
- `missingfile`: несуществующий `key_files` → `InvalidArgument`;
- в основном процессе: ключ из PEM загружается; тот же ключ из файла даёт
  **тот же отпечаток** и не удваивает набор; живое подключение с ключом,
  заданным только текстом, прошло, и `shell` по нему выполнился.
